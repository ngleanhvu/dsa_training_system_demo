...
public interface ContestParticipantRepo extends JpaRepository<ContestParticipant, Integer> {
    ...
    SELECT
      ...
      u.email,
      cb.problem_id,
      CASE WHEN COUNT(CASE WHEN cs.is_accepted = true THEN 1 END) > 0 THEN true ELSE false END AS solved,
      COUNT(CASE WHEN cs.is_accepted = false
           AND (SELECT COUNT(*) FROM contest_submissions cs2
        ...
        THEN 1 ELSE NULL END) AS wrong_count,
    ...
    LEFT JOIN submissions s
           ON s.submission_id = cs.submission_id
